 ...
 * Return: Modified input string with comments removed, or NULL if the whole line is a comment.
 */
char *without_comment(char *in)
{
	...
	if (up_to != 0)
	{
		in = _realloc(in, i, up_to + 1); // Adjust the size of the input string
		in[up_to] = '\0'; // Null-terminate the string at the position before the comment
	}
...
void shell_loop(data_shell *datash)
{
	...
	while (loop == 1)
	{
		write(STDIN_FILENO, "^-^ ", 4); // Print the prompt
